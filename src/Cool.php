...
class Cool{
    ...
    /**
     * 环境配置
     * @var \Cool\Foundation\Environment
     */
    public static $env;
    ...
    public static function configure(array $config)
    {
        foreach ($config as $key => $value) {
            ...
            if (is_array($value)) {
                ...
                // 实例化
                return \Cool\Foundation\Bean::newInstance($config['ref']);
            }
        }
        return $config;
    }
    ...
    public static function importProperties($object, $config)
    {
        foreach ($config as $name => $value) {
            ...
            if (!interface_exists($var) && !class_exists($var)) {
                throw new \Cool\Foundation\Exceptions\DependencyInjectionException("Interface or class not found, class: {$class}, property: {$name}, @var: {$var}");
            }

            foreach ($value as $v){
                if (!($v instanceof $var)) {
                    throw new \Cool\Foundation\Exceptions\DependencyInjectionException("The type of the imported property does not match, class: {$class}, property: {$name}, @var: {$var}");
                }
            }
        }
    ...
    {
        $env = new \Cool\Foundation\Environment(['filename' => $filename]);
        $env->load();
        self::$env = $env;
        return true;
    }
