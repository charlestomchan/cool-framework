...
use Cool\Foundation\Exceptions\ErrorException;
...
class Error
{
    ...
    public static function appError($errno, $errstr, $errfile = '', $errline = 0)
    {
        if (error_reporting() & $errno) {
            ...
            if (static::isFatalWarning($errno, $errstr)) {
                static::appException(new ErrorException($errno, $errstr, $errfile, $errline));
                return;
            }
            // 转换为异常抛出
    ...
    public static function appShutdown()
    {
        if (!is_null($error = error_get_last()) && static::isFatal($error['type'])) {
            // 委托给异常处理
            static::appException(new ErrorException($error['type'], $error['message'], $error['file'], $error['line']));
        }
    }
