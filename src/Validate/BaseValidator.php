...
class BaseValidator extends AbstractObject
{
    ...
    public function validate()
    {
        ...
        if (!$result) {
            ...
            if ($this instanceof \Cool\Validate\FileValidator) {
                // 实例化文件对象
                $this->mainValidator->$attribute = \Cool\Http\Message\UploadFile::newInstance($attribute);
            } else {
                // 属性赋值
    ...
    protected function scalar()
    {
        ...
        if (!is_null($value) && !is_scalar($value)) {
            ...
            // 回调验证器忽略该类型的验证
            if ($this instanceof \Cool\Validate\CallValidator) {
                return true;
            }
            // 设置错误消息
