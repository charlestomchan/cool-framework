...
class WebSocketServer extends AbstractServer
{
    ...
    protected $_defaultSetting = [
        ...
        'task_worker_num'        => 0,
        // PID 文件
        'pid_file'               => '/var/run/cool-websocketd.pid',
        // 日志文件路径
        'log_file'               => '/tmp/cool-websocketd.log',
        // 异步安全重启
        'reload_async'           => true,
        // 退出等待时间
        'max_wait_time'          => 60,
    ...
    public function onHandshake(\Swoole\Http\Request $request, \Swoole\Http\Response $response)
    {
        ...
        try {
            ...
            \Cool::$app->ws->beforeInitialize($this->server, $fd);
            \Cool::$app->registry->beforeInitialize($fd);
            // 拦截
            \Cool::$app->runHandshake(\Cool::$app->ws, \Cool::$app->request, \Cool::$app->response);
            // 执行回调
    ...
    public function onOpen(\Swoole\WebSocket\Server $server, \Swoole\Http\Request $request)
    {
        ...
        try {
            ...
            // 前置初始化
            \Cool::$app->request->beforeInitialize($request);
            \Cool::$app->ws->beforeInitialize($server, $fd);
            \Cool::$app->registry->beforeInitialize($fd);
            // 处理消息
            \Cool::$app->runOpen(\Cool::$app->ws, \Cool::$app->request);
            // 执行回调
    ...
    public function onMessage(\Swoole\WebSocket\Server $server, \Swoole\WebSocket\Frame $frame)
    {
        ...
        try {
            ...
            \Cool::$app->registry->beforeInitialize($fd);
            // 处理消息
            \Cool::$app->runMessage(\Cool::$app->ws, new Frame($frame));
            // 执行回调
    ...
    public function onClose(\Swoole\WebSocket\Server $server, int $fd, int $reactorId)
    {
        ...
        try {
            ...
            \Cool::$app->registry->beforeInitialize($fd);
            // 处理连接关闭
            \Cool::$app->runClose(\Cool::$app->ws);
            \Cool::$app->registry->afterInitialize();
            // 执行回调
